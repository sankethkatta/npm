...
function tryClone (from, combinedURL, silent, cb) {
  ...
  // semver treeish can contain special characters which are escaped when the
...
function resolveHead (from, cloneURL, treeish, cachedRemote, cb) {
  if (semver.validRange(treeish)) {
    resolveSemverTreeish(from, cloneURL, treeish, cachedRemote, cb)
  } else {
    resolveTreeish(from, cloneURL, treeish, cachedRemote, cb)
  }
}

// resolve a treeish which represents a valid semver range with the max
// satisfying tag
function resolveSemverTreeish (from, cloneURL, treeish, cachedRemote, cb) {
  log.verbose('resolveSemverTreeish', from, 'original treeish:', treeish)
  git.whichAndExec(
    ['tag', '--list'],
    { cwd: cachedRemote, env: gitEnv() },
    function (er, stdout, stderr) {
      ...
      var resolvedTreeish = semver.maxSatisfying(tags, treeish)
      resolvedTreeish = resolvedTreeish || treeish
      log.silly('resolveSemverTreeish', from, 'resolved treeish:', resolvedTreeish)

      resolveTreeish(from, cloneURL, resolvedTreeish, cachedRemote, cb)
    }
  )
}
